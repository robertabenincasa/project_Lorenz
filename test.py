...
@given(r = st.floats(min_value = 1., max_value= 24., allow_nan=False,
        exclude_min = True),
        eps = exnp.arrays(np.dtype(float), N ,elements = 
        st.floats(min_value = 1E-7,max_value= 1.,allow_nan=False)),  
        which_variable = st.integers(min_value = 0,
        max_value= 2))
@settings(max_examples = 10, deadline=None)     
def test_stability(r,eps,which_variable):
    ...
    if r < sigma*(sigma+b+3)/(sigma-b-1):
        ...
        sol_1 = lorenz.integration_Lorenz_system(lorenz.lorenz,NUM_STEPS, t, IC_1, set_)
        
        IC_2 = lorenz.perturbation(IC0_2,eps,which_variable)
        
        sol_2 = lorenz.integration_Lorenz_system(lorenz.lorenz,NUM_STEPS, t, IC_2, set_)
        
        assert np.allclose(sol_1[NUM_STEPS-10:NUM_STEPS,:,:], point_1.T, rtol=2.)
    
        assert np.allclose(sol_2[NUM_STEPS-10:NUM_STEPS,:,:], point_2.T, rtol=2.)
